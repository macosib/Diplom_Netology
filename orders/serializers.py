...
class BasketSerializer(serializers.ModelSerializer):
        ...
        for item in items:
            ...
            if quantity > product.quantity:
                raise serializers.ValidationError(
                    {
                        "status": "Failure",
                        "error": "There is not enough available product",
                    }
                )
            if quantity <= 0:
                raise serializers.ValidationError(
                    {
                        "status": "Failure",
                        "error": "The quantity of the product must be greater than 0",
                    }
                )
        return data
    ...
    def create(self, validated_data):
        ...
        order, _ = Order.objects.get_or_create(
            **validated_data, user_id=user.id, state="basket"
        )
    ...
    def validate(self, data):
        ...
        if not contact:
            raise serializers.ValidationError(
                {
                    "status": "Failure",
                    "error": "To confirm the order, you need to add contacts",
                }
            )
        if not order:
            raise serializers.ValidationError(
